class Result
  def initialize
    # Создадим массив результатов теста
    @results = [
      # >= 30 баллов
      ...
      "дискомфорта. Эта особенность характера не ускользнула от вашего внимания, " \
      "и вы часто сердитесь на себя за нее. Вспомните, как легко вы общаетесь, " \
      "когда чем то очень увлечены. Ведь можете же, когда хотите.",
      # 19-24 балла
      ...
      "ощущений. Но многолюдные места, шумные компании и болтливые люди вызывают " \
      "у вас раздражение.",
      # 9-13 балла
      "Вы весьма общительны, не так ли? Ваши любопытство, разговорчивость, " \
      "вспыльчивость вызывают у некоторых людей раздражение и заставляют думать " \
      "о вас как о легкомысленном человеке. Вы легко знакомитесь с новыми " \
      "людьми, вам нравятся большие компании, особенно если вынаходитесь в " \
      "центре всеобщего внимания. Вам не помешает выработать в себе немного " \
      "терпения и усидчивости, необходимых для решения серьезных проблем.",
      # 4-8 балла
...
# Выводим результат, в зависимости от полученных баллов
  def get_result(test)
    if test.answers >= 30
      puts @results[0]
    elsif test.answers >= 25
      puts @results[1]
    elsif test.answers >= 19
      puts @results[2]
    elsif test.answers >= 14
      puts @results[3]
    elsif test.answers >= 9
      puts @results[4]
    elsif test.answers >= 4
      puts @results[5]
    else
      puts @results[6]
    end
